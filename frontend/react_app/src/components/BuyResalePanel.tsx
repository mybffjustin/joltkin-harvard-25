...
import { useAlgod, ROUTER_APP_ID, TICKET_ASA_ID } from "../lib/algorand";
import { signSubmitConfirm } from "../walletManager";

/**
 * Fee notes:
 * - App calls that execute inner transactions must pay for inner ops.
 * - We set a flat fee on the AppCall; tune these for your router.
 */
const BUY_APP_FEE_UALGOS = 3000; // ~3 inner payments
const RESALE_APP_FEE_UALGOS = 2000;
...
export default function BuyResalePanel() {
  ...
  // Derived: app escrow address (force to string for rendering)
  const appAddr: string = useMemo(() => {
    try {
      return appId ? String(algosdk.getApplicationAddress(appId)) : "";
    } catch {
      return "";
    }
  }, [appId]);
  ...
  const canSubmit =
    haveWallet && numericOk && !!appAddr && !!buyerAddr && !!sellerAddr && !busy;

  const formatAlgo = (u: number) =>
    (u / 1_000_000).toLocaleString(undefined, { maximumFractionDigits: 6 });

  // Helper: set flat fee in a bigint/number-safe way
  ...
  async function run(which: Flow) {
    try {
      ...
      const appCall = algosdk.makeApplicationNoOpTxnFromObject({
        ...
        appArgs: [new TextEncoder().encode(which)],
      ...
      // Group, sign via active wallet, submit, confirm
      const { txid } = await signSubmitConfirm([appCall, pay, asaXfer]);
  ...
  return (
    <div className="card" aria-busy={!!busy}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
        <div>
          ...
          <div style={{ fontSize: 12, opacity: 0.7, marginTop: 4 }}>
            App address: <code>{appAddr ? String(appAddr) : "—"}</code>
          </div>
        </div>
          ...
          <div>
            ...
            {buyerAddr && buyerOptedIn === false && (
              <div style={{ color: "#c00", fontSize: 12, marginTop: 4 }}>
                Buyer is <strong>not opted-in</strong> to ASA {asaId}. They must opt-in before
      ...
      <p style={{ marginTop: 12, fontSize: 12, opacity: 0.75, lineHeight: 1.35 }}>
        <strong>Demo caveat:</strong> This UI can run in “self-transfer” mode for convenience. In
        production, the <em>seller</em> (current holder) must sign the ASA transfer, and the{" "}
        <em>buyer</em> pays the ALGO price. Your wallet integration should gather both signatures
